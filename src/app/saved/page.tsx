    ...
    <div className="min-h-screen bg-neutral-50 p-6">
      ...
      <div className="max-w-4xl mx-auto">
        <div className="flex items-center gap-4 mb-8">
          ...
          <h1 className="text-2xl font-bold">📚 {t.savedRecipes} ({recipes.length})</h1>
        </div>

        {loading && <p className="text-center text-neutral-400">{t.loading}</p>}
        {!loading && recipes.length === 0 && (
          <p className="text-center text-neutral-400">{t.noSavedRecipes}</p>
        )}
            ...
            <div key={r._id} className="bg-white rounded-2xl border border-neutral-200 shadow-sm overflow-hidden">
              ...
              <div className="flex justify-between items-center p-5">
                <div>
                  ...
                  <div className="flex gap-3 text-sm text-neutral-500 mt-1">
                    <span>⏱ {lang === 'en' ? r.prepTime.replace(/phút/g, 'mins') : r.prepTime}</span>
                    <span>🔥 {lang === 'en' ? r.cookTime.replace(/phút/g, 'mins') : r.cookTime}</span>
                    <span>👨‍🍳 {lang === 'en' ? r.difficulty.replace(/Dễ/g, 'Easy').replace(/Trung bình/g, 'Medium').replace(/Khó/g, 'Hard') : r.difficulty}</span>
                  </div>
                </div>
              ...
              {expanded === r._id && (
                <div className="border-t border-neutral-100 p-5 grid grid-cols-1 md:grid-cols-3 gap-6">
                  <div>
                    <h3 className="font-bold mb-3 text-orange-500">🧂 {t.ingredientsTitle}</h3>
                  ...
                  <div className="md:col-span-2">
                    <h3 className="font-bold mb-3 text-orange-500">👨‍🍳 {t.instructionsTitle}</h3>
              ...
              <div className="px-5 pb-3">
                <p className="text-xs text-neutral-400">{new Date(r.createdAt).toLocaleDateString(lang === 'vi' ? "vi-VN" : "en-US")}</p>
              </div>
            </div>
          ))}
        </div>
